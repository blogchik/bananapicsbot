...
async def test_dynamic_price_seedream(mock_settings):
    price = await _dynamic_price_for_model(
        model_key="seedream-v4", size="1024*1024", resolution=None, quality="standard", settings=mock_settings
    )
    # 0.027 * 1000 = 27
    assert price == 27
...
async def test_dynamic_price_nano_banana_pro_4k(mock_settings):
    price = await _dynamic_price_for_model(
        model_key="nano-banana-pro", size=None, resolution="4K", quality=None, settings=mock_settings
    )
    # 0.24 * 1000 = 240
    assert price == 240
...
async def test_dynamic_price_nano_banana_pro_default(mock_settings):
    price = await _dynamic_price_for_model(
        model_key="nano-banana-pro", size=None, resolution="FHD", quality=None, settings=mock_settings
    )
    # 0.14 * 1000 = 140
    assert price == 140
